...
class BeltItem(QGraphicsPathItem):
    ...
    def paint(
        ...
        # Only show when flow rate visualization is enabled
        if self.canvas.show_flow_rate and self._flow_rate is not None and not self.is_over_capacity:
        ...
        # Only show when flow rate visualization is enabled
        if self.canvas.show_flow_rate and self.is_over_capacity:
