...
def get_arc_points(
    C: QPointF, phi_start: float, phi_end: float, ccw: bool, R: float, num_points: int = 30
) -> list[QPointF]:
...
def get_dubins_path(
    S: QPointF, theta_s: float, E: QPointF, theta_e: float, R: float
) -> dict | None:
    """
    ...
    def compute_candidate(
        C1: QPointF, C2: QPointF, type1: str, type2: str, external: bool
    ) -> dict | None:
...
class ArcTestView(QGraphicsView):
    def __init__(self):
        ...
        # Draw origin marker
        self.start_marker = self.scene.addEllipse(
            -5, -5, 10, 10, QPen(Qt.GlobalColor.red), QColor(255, 0, 0)
        )
        ...
        self.circle1 = self.scene.addEllipse(
            -self.radius,
            -self.radius,
            self.radius * 2,
            self.radius * 2,
        ...
        self.circle2 = self.scene.addEllipse(
            -self.radius,
            -self.radius,
            self.radius * 2,
            self.radius * 2,
        ...
        self.t1_marker = self.scene.addEllipse(
            -4, -4, 8, 8, QPen(Qt.GlobalColor.black), QBrush(Qt.GlobalColor.yellow)
        )
        self.t2_marker = self.scene.addEllipse(
            -4, -4, 8, 8, QPen(Qt.GlobalColor.black), QBrush(Qt.cyan)
        )
    ...
    def _compute_and_draw(self, end: QPointF):
        ...
        if best:
            ...
            arc1_points = get_arc_points(
                best["C1"], best["phi1_s"], best["phi1_p"], ccw1, self.radius
            )
            ...
            arc2_points = get_arc_points(
                best["C2"], best["phi2_q"], best["phi2_e"], ccw2, self.radius
            )
