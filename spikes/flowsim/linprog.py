"""Pure Python Simplex Method for Linear Programming.

Source: https://github.com/dmishin/pylinprog
License: MIT

Vendored for packaging size testing.
"""
...
class NumberTypeclass:
    ...
    def nonnegative(self, x):
        return self.positive(x) or self.iszero(x)

    def coerce(self, x):
        return x

    def coerce_vec(self, x):
        return [self.coerce(xi) for xi in x]

    def coerce_mtx(self, x):
        return [self.coerce_vec(xi) for xi in x]
...
class SimplexSolver:
    ...
    def _validate_diagonalized(self):
        for i in self.basis:
                ...
                if j1 == j:
                    ...
                    if not self.numclass.iszero(a_j[i]):
                        raise AssertionError(
                            "A: column {i} row {j} must be 0".format(**locals())
                        )
        for bi in self.b:
        ...
        for j, b_j in enumerate(self.b):
            ...
            if self.numclass.iszero(b_j):
                if not self.numclass.positive(a_ji):
                    continue
            ratio = b_j / a_ji
            if not self.numclass.nonnegative(ratio):
                continue
    ...
    def show(self):
        ...
        print("Basis:", ", ".join(f"x{i}" for i in self.basis))
        self._show_row("", (f"x{i}" for i in range(self.n)))
...
def linsolve(
    ...
    Arguments:
        ...
        eq_right: M'-vector B'
        nonneg_variables: list of variables (0-based indices) that are >= 0
        num: instance of NumberTypeclass, defining numeric implementation.
    ...
    def positivise_row(row):
        return [
            row[i] if i not in negative_part2positive_part
            else -row[negative_part2positive_part[i]]
